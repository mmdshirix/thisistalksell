// Central DB helper and high-level queries using @vercel/postgres.
// NOTE: Do NOT throw at import-time if env is missing; this must be safe during next build.

import { sql } from "@vercel/postgres"

export { sql }

// Provide a runtime getter compatible with routes that expect getSql()
...
// Database connection test function (safe: no import-time env checks)
...
// Basic CRUD and analytics helpers (used across routes & pages)
...
export async function getChatbots(): Promise<any[]> {
  try {
    ...
    // Attempt minimal auto-fix for missing columns, then retry
...
export async function getUniqueUsersCount(chatbotId: number): Promise<number> {
  try {
    const result = await sql`
      ...
      WHERE chatbot_id = ${chatbotId}
    `
    return Number(result.rows[0]?.unique_users || 0)
  } catch (error) {
...
export async function getAverageMessagesPerUser(chatbotId: number): Promise<number> {
  try {
    const result = await sql`
      SELECT 
        ROUND(COUNT(*)::numeric / NULLIF(COUNT(DISTINCT user_ip), 0), 2) as avg_messages
      FROM chatbot_messages 
      WHERE chatbot_id = ${chatbotId}
    `
    return Number(result.rows[0]?.avg_messages || 0)
  } catch (error) {
